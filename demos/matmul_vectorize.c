...
void kernel_16x6(float* blockA, float* blockB, float* C, const int M, const int N, const int K) {
    __m256 C_buffer[6][2];
    __m256 b_packFloat8;
    ...
    for (int p = 0; p < K; p++) {
        a0_packFloat8 = _mm256_loadu_ps(&blockA[p * M]);
        a1_packFloat8 = _mm256_loadu_ps(&blockA[p * M + 8]);
